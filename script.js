...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const defaultMessage = [
    ...
    '',
    'Estoy interesado en implementar un sistema, un Agente IA o automatizar un proceso manual.',
  ...
  const observed = document.querySelectorAll(
    '.product-card, .enterprise-card, .ai-card, .ai-process, .sector-pills span, .about-section h2, .about-section p, .hero-left h1, .hero-left .lead, .contact-panel, .legal-content section'
  );
  ...
  leadForm?.addEventListener('submit', event => {
    ...
    if (!leadForm.checkValidity()) {
      ...
      setFormStatus('Completa los campos requeridos para enviar tu solicitud.', 'is-error');
      return;
    }
    ...
    if (!endpoint) {
      ...
      setFormStatus('Abrimos WhatsApp para enviar tu solicitud. Activa el Apps Script para guardarla también en Google Sheets.', 'is-success');
      return;
    }
